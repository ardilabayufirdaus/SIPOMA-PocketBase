...
# --- OMNI PREDATOR V8.2 (STABILITY EDITION) ---
...
# Memory for V8.2 Cooldown & Stability
COOLDOWN_DICT = {} # {pair: last_close_time}
...
def fetch_yfinance_data(pair):
    ...
    try:
        ...
        # Calculate ADX reliably
        pdm, mdm = h4['High'].diff(), h4['Low'].diff().abs()
        pdm[pdm < 0] = 0; mdm[mdm < 0] = 0
        atr_h4 = h4_tr.rolling(14).mean()
        pdi = 100 * (pdm.rolling(14).mean() / (atr_h4 + 1e-9))
        mdi = 100 * (mdm.rolling(14).mean() / (atr_h4 + 1e-9))
        adx_h4_num = 100 * (abs(pdi - mdi) / (pdi + mdi + 1e-9)).rolling(14).mean().iloc[-1]
        ...
        smc_m15 = detect_smc_patterns(m15.tail(30))
...
async def trader_loop():
    ...
    await send_telegram(f"🔱 **OMNI PREDATOR V8.2** 🔱\n🛡️ **STABILITY EDITION ACTIVATED**\n- Rule: No 1-Second Stop-Outs\n- Rule: 30-Min Cooldown Post-Loss\n- Rule: Max 1 Cluster Exposure")
    ...
    while True:
        try:
            ...
            for pair in PAIRS:
                ...
                # 2. V8.2 COOLDOWN GUARD (30 Minutes after last potential stop out)
                if pair in COOLDOWN_DICT:
                    if time.time() - COOLDOWN_DICT[pair] < 1800:
                        logging.info(f"COOLDOWN: Skipping {pair} for stability.")
                        continue

                # 3. CORRELATION GUARD (Strict)
                ...
                # MANDATE: V8.2 Minimum SL distances to prevent Stops Level violations
                min_sl = 50 if pair == "XAUUSD" else 30 
                
                prompt = f"""
                🔱 MISSION: OMNI PREDATOR V8.2 (STABILITY EDITION) 🔱
                PAIR: {pair} | DATA: {market_data['formatted']} | NEWS: {news} | COT: {cot_data}
                
                [V8.2 STABILITY MANDATES - AKURASI >= 95%]:
                1. STOP-LOSS SAFETY: Jarak SL WAJIB >= {min_sl} pips agar tidak terkena "Stops Level" atau "Noise".
                2. STRUCTURE BASE: Gunakan high/low H1/H4 untuk SL utama. Jangan entry jika posisi terlalu dekat dengan SL.
                3. ATR BREATHING: Beri ruang bernapas pada entry. No 1-second stop outs allowed!
                4. FUNDAMENTAL SCORE: Wajib >= |7| untuk A+ signal.
                
                OUTPUT JSON: {{ 
                    "decision": "BUY_MARKET"|"SELL_MARKET"|"HOLD", 
                    "style": "string", "f_score": int, "confidence": int,
                    "sl_pips": int, "tp_pips": int, "risk_percent": float,
                    "reason": "Detail 12-layer + Stability verification"
                }}
                """
                resp = client.chat.completions.create(model=MODEL_ID, messages=[{'role': 'system', 'content': 'Anda adalah OMNI PREDATOR V8.2. Fokus pada stabilitas dan akurasi ekstrim (A++). Pastikan SL cukup jauh untuk memberi ruang bernapas.'}, {'role': 'user', 'content': prompt}], response_format={'type': 'json_object'})
                res = json.loads(resp.choices[0].message.content)
                dec = str(res.get('decision', 'HOLD')).upper()
                
                # V8.2 Guard: Force Minimum SL distance in logic
                sl_pips = max(int(res.get('sl_pips', min_sl)), min_sl)
                tp_pips = int(res.get('tp_pips', sl_pips * 2))
                ...
                if (('BUY' in dec and market_data['h4_trend'] == 'BULLISH') or ('SELL' in dec and market_data['h4_trend'] == 'BEARISH')) and abs(res.get('f_score', 0)) >= 7 and res.get('confidence', 0) >= 95:
                    with open(SIGNAL_PATH, 'w') as f: f.write(f"{pair}|{dec}|{market_data['current_price']}|{sl_pips}|{tp_pips}|{res['risk_percent']}")
                    conn = sqlite3.connect(DB_PATH); conn.execute('INSERT INTO trade_history (pair, action, entry, sl, tp, risk, timestamp, reason, score, style) VALUES (?,?,?,?,?,?,?,?,?,?)', (pair, dec, market_data['current_price'], sl_pips, tp_pips, res['risk_percent'], datetime.now().strftime('%Y-%m-%d %H:%M:%S'), res['reason'], res['f_score'], res['style'])); conn.commit(); conn.close()
                    COOLDOWN_DICT[pair] = time.time() # Start cooldown
                    await send_telegram(f"🛡️ **OMNI PREDATOR V8.2 STABLE EXECUTION**\n💰 Style: {res.get('style')}\n📊 F-Score: {res.get('f_score')}\n🎯 Action: {dec} {pair}\n📍 Entry: {market_data['current_price']}\n🛡️ SL: {sl_pips} pips | TP: {tp_pips} pips\n💡 Reason: {res['reason']}")
                    await asyncio.sleep(5)
            await asyncio.sleep(60)
        except Exception as e: logging.error(f"V8.2 Error: {e}"); await asyncio.sleep(30)
