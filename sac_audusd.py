...
1. TECHNICAL: Focus on EMA 20/50/200 Trend in D1/H4. SMA 200 Weekly. RSI (14) Divergence in D1. MACD for trend reversal and continuation. ADX > 25 for valid trend, < 20 avoid (ranging).
2. FUNDAMENTAL: AUD is a commodity currency (Iron ore, Copper). Correlated with China risk sentiment. Monitor RBA rate, China PMI, Iron ore price.
3. SENTIMENT: Risk-on/Risk-off driven. Check correlation with XAU & copper (bullish bias if both rise). VIX levels impact AUDUSD.
4. RISK CONTROL: ATR H4 35-70 pip. Fibonacci 61.8% pullback is ideal. Daily Pivot + S/D zone confluence. Round numbers (0.6500, 0.7000, 0.7500) are institutional attractors.
...
def get_tf_indicators(df, tf_id):
    ...
    return {
        ...
        "ema200": ema200, "sma200": sma200, "rsi": rsi, "macd": macd.to_dict(), 
        "adx": adx, "atr": atr, "trend": trend
    }
...
async def process_multi_tf_scan():
    ...
    try:
        ...
        if decision.get('action', 'HOLD') != 'HOLD':
            # Long-term trend focus on AUDUSD
            daily_trend = tf_matrix.get('TF_1440', {}).get('trend')
            alignment = True
            if 'BUY' in decision['action'] and daily_trend == 'Bearish': alignment, msg = False, "Blocked: Counter Daily Trend"
            if 'SELL' in decision['action'] and daily_trend == 'Bullish': alignment, msg = False, "Blocked: Counter Daily Trend"
            if alignment and decision['confidence'] >= 83:
                ...
                asyncio.create_task(analyze_and_send(SAC_VERSION, f"🛡️ Trend Daily: {daily_trend}\n💎 Order: {PAIR} {decision['action']}\nAudit: {decision.get('reason','')}\n"))
