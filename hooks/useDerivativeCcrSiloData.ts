...
import { useDerivativeSiloCapacities as useSiloCapacities } from './useDerivativeSiloCapacities';
import { formatDateToISO8601 } from '../utils/dateUtils';
import { pb } from '../utils/pocketbase-simple';

/**
 * Type definitions based on the schema for Derivative Material Storage Data
 */
...
/**
 * Hook for managing Derivative CCR Silo data with optimized CRUD operations
 * Targeting derivative_ccr_silo_data
 */
export const useDerivativeCcrSiloData = () => {
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<Error | null>(null);

  const pendingOperations = 0;

  // Get Derivative silo capacities for reference data
  ...
  const getDataForDatePaginated = useCallback(
    async (params: SiloDataParams) => {
      ...
      try {
        ...
        const result = await pb.collection('derivative_ccr_silo_data').getList(page, perPage, {
          filter,
          sort: 'created',
          expand: 'silo_id',
        });
  ...
  const getDataForDate = useCallback(
    async (date: string, unit_id?: string) => {
      ...
      try {
        ...
        const items = await pb.collection('derivative_ccr_silo_data').getFullList({
          filter,
          sort: 'created',
          expand: 'silo_id',
        });
    ...
    async (
      ...
      if (
        ...
        try {
          ...
          const records = await pb.collection('derivative_ccr_silo_data').getFullList({
            filter,
            sort: '-created',
            expand: 'silo_id',
          });
          ...
          if (filteredRecords.length === 0) {
            return await pb.collection('derivative_ccr_silo_data').create({
              date: formattedDate,
              silo_id: siloId,
              ...data,
            });
          ...
          return await pb.collection('derivative_ccr_silo_data').update(record.id, data);
        } catch (err) {
      ...
      // Method 2: Update by ID directly
        ...
        try {
          return await pb.collection('derivative_ccr_silo_data').update(id, data);
        } catch (err) {
          setError(err instanceof Error ? err : new Error('Failed to update silo data'));
          throw err;
        }
      ...
      // Method 3: Update specific shift field
        ...
        try {
          ...
          if (filteredRecords.length === 0) {
            ...
            const record = filteredRecords[0];
            return await pb.collection('derivative_ccr_silo_data').update(record.id, updateData);
          }
        } catch (err) {
  ...
  const createSiloData = useCallback(
    async (data: Partial<CcrSiloDataSchema> | Record<string, unknown>) => {
      try {
        ...
        return await pb.collection('derivative_ccr_silo_data').create(data);
      } catch (err) {
  ...
  const deleteSiloData = useCallback(
    async (idOrDate: string, siloId?: string, shift?: string, field?: string, unitId?: string) => {
      ...
      if (!siloId) {
        ...
        try {
          await pb.collection('derivative_ccr_silo_data').delete(idOrDate);
          return { success: true };
        } catch (err) {
        ...
        try {
          ...
          if (shift && field) {
            ...
            if (!hasOtherField && !hasOtherShifts) {
              ...
              await pb.collection('derivative_ccr_silo_data').update(record.id, updateData);
