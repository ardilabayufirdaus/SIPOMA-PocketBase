...
/**
 * Hook for managing Derivative CCR Material Usage data with CRUD operations
 * Targeting derivative_ccr_material_usage
 */
export const useDerivativeCcrMaterialUsage = () => {
  ...
  const getDataForDatePaginated = useCallback(async (params: MaterialUsageParams) => {
    ...
    try {
      ...
      const result = await pb.collection('derivative_ccr_material_usage').getList(page, perPage, {
        filter,
        sort: 'created',
      });
  ...
  const getDataForDate = useCallback(
    async (date: string, plant_unit?: string, plant_category?: string) => {
      ...
      try {
        ...
        const items = await pb.collection('derivative_ccr_material_usage').getFullList({
          filter,
          sort: 'created',
        });
  ...
  const createMaterialUsage = useCallback(async (data: Partial<CcrMaterialUsageSchema>) => {
    try {
      ...
      return await pb.collection('derivative_ccr_material_usage').create(data);
    } catch (err) {
    ...
    async (
      ...
      if (typeof idOrDateOrParams === 'string' && typeof dataOrPlantUnit === 'object') {
        ...
        try {
          ...
          if (
            ...
            const currentRecord = await pb.collection('derivative_ccr_material_usage').getOne(id);
            const total =
          ...
          return await pb.collection('derivative_ccr_material_usage').update(id, data);
        } catch (err) {
      ...
      // Method 2: Update by date and plant_unit
        ...
        try {
          ...
          const records = await pb.collection('derivative_ccr_material_usage').getFullList({
            filter,
            sort: '-created',
          });
  ...
  const deleteMaterialUsage = useCallback(async (id: string) => {
    try {
      return await pb.collection('derivative_ccr_material_usage').delete(id);
    } catch (err) {
  ...
  const saveMaterialUsage = useCallback(async (data: Partial<CcrMaterialUsageSchema>) => {
    ...
    try {
      ...
      const existingRecords = await pb.collection('derivative_ccr_material_usage').getFullList({
        filter,
      });
      ...
      if (existingRecords.length > 0) {
        ...
        return await pb.collection('derivative_ccr_material_usage').update(record.id, saveData);
      } else {
        // Create new record
        return await pb.collection('derivative_ccr_material_usage').create(saveData);
      }
    } catch (err) {
