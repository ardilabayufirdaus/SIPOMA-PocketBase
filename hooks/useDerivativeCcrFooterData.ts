...
export const useDerivativeCcrFooterData = () => {
  const saveFooterData = useCallback(async (footerData: CcrFooterData) => {
    ...
    const data = {
      ...
      plant_unit: footerData.plant_unit || 'Derivative',
    ...
    const existingRecords = await safeApiCall(
      () =>
        pb.collection('derivative_ccr_footer_data').getFullList({
          filter: `date="${footerData.date}" && parameter_id="${footerData.parameter_id}" && plant_unit="${footerData.plant_unit || 'Derivative'}"`,
        }),
    ...
    if (existingRecords.length > 0) {
      ...
      const updatedRecord = await safeApiCall(
        () => pb.collection('derivative_ccr_footer_data').update(existingId, data),
        { retries: 3, retryDelay: 3000, handleNetworkChange: true }
      );
      if (!updatedRecord) {
        return null;
      ...
      const record = await safeApiCall(
        () => pb.collection('derivative_ccr_footer_data').create(data),
        {
          retries: 3,
          retryDelay: 3000,
    ...
    async (date: string, parameterId: string, plantUnit?: string) => {
      ...
      if (records) {
        for (const record of records) {
          await safeApiCall(() => pb.collection('derivative_ccr_footer_data').delete(record.id), {
            retries: 2,
            retryDelay: 2000,
          });
        }
