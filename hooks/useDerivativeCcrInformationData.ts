...
export const useDerivativeCcrInformationData = (selectedDate?: string) => {
  ...
  const {
    ...
    queryKey: ['derivative-ccr-information'],
    queryFn: async (): Promise<CcrInformationData[]> => {
      try {
        const records = await safeApiCall(() =>
          pb.collection('derivative_ccr_information').getFullList({
            sort: '-date',
          })
        );
  ...
  const informationData = rawInfoData || [];
  ...
  const saveInformationMutation = useMutation({
    mutationFn: async (data: { date: string; plantUnit: string; information: string }) => {
      ...
      if (existingRecord) {
        ...
        result = await safeApiCall(() =>
          pb.collection('derivative_ccr_information').update(existingRecord.id, {
        ...
        // Create new record
        result = await safeApiCall(() =>
          pb.collection('derivative_ccr_information').create({
            date: formattedDate,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['derivative-ccr-information'] });
    },
  });
