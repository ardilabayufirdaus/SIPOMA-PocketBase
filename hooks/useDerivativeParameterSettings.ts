...
const CACHE_KEY = CacheKeys.DERIVATIVE_PARAMETER_SETTINGS;
const CACHE_TIME = 30; // Minutes

export const useDerivativeParameterSettings = () => {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      const result = await safeApiCall(() =>
        pb.collection('derivative_parameter_settings').getFullList({
          sort: 'parameter',
        })
      );
    ...
    const subscribe = async () => {
      try {
        ...
        unsubPromise = await safeApiCall(() =>
          pb.collection('derivative_parameter_settings').subscribe('*', (e) => {
            if (!isSubscribed) return;
  ...
  const addRecord = useCallback(
    async (record: Omit<ParameterSetting, 'id'>) => {
      ...
      try {
        await pb.collection('derivative_parameter_settings').create(cleanedRecord);
        cacheManager.delete(CACHE_KEY);
        fetchRecords();
      } catch {
        /* ignore */
  ...
  const updateRecord = useCallback(
    async (updatedRecord: ParameterSetting) => {
      ...
      try {
        await pb.collection('derivative_parameter_settings').update(id, cleanedUpdateData);
        cacheManager.delete(CACHE_KEY);
        fetchRecords();
      } catch {
        /* ignore */
  ...
  const deleteRecord = useCallback(
    async (recordId: string) => {
      try {
        await pb.collection('derivative_parameter_settings').delete(recordId);
        cacheManager.delete(CACHE_KEY);
        fetchRecords();
      } catch {
        /* ignore */
  ...
  const setAllRecords = useCallback(
    async (newRecords: Omit<ParameterSetting, 'id'>[]) => {
      try {
        const existingRecords = await pb.collection('derivative_parameter_settings').getFullList();
        if (existingRecords && existingRecords.length > 0) {
          for (const record of existingRecords) {
            await pb.collection('derivative_parameter_settings').delete(record.id);
          }
        }
        if (newRecords.length > 0) {
          for (const record of newRecords) {
            await pb.collection('derivative_parameter_settings').create(record);
          }
        }
        cacheManager.delete(CACHE_KEY);
        fetchRecords();
