...
const CACHE_KEY = CacheKeys.DERIVATIVE_SILO_CAPACITIES;
const CACHE_TIME = 15; // Minutes

export const useDerivativeSiloCapacities = () => {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      const result = await pb.collection('derivative_silo_capacities').getFullList({
        sort: 'silo_name',
      });
    ...
    const subscribe = async () => {
      try {
        ...
        unsubPromise = await safeApiCall(() =>
          pb.collection('derivative_silo_capacities').subscribe('*', (e) => {
            if (!isSubscribed) return;
  ...
  const addRecord = useCallback(
    async (record: Omit<SiloCapacity, 'id'>) => {
      try {
        await pb.collection('derivative_silo_capacities').create(record);
        cacheManager.delete(CACHE_KEY);
        fetchRecords();
      } catch {
        /* ignore */
  ...
  const updateRecord = useCallback(
    async (updatedRecord: SiloCapacity) => {
      try {
        ...
        await pb.collection('derivative_silo_capacities').update(id, updateData);
        cacheManager.delete(CACHE_KEY);
        fetchRecords();
      } catch {
        /* ignore */
  ...
  const deleteRecord = useCallback(
    async (recordId: string) => {
      try {
        await pb.collection('derivative_silo_capacities').delete(recordId);
        cacheManager.delete(CACHE_KEY);
        fetchRecords();
      } catch {
        /* ignore */
  ...
  const setAllRecords = useCallback(
    async (newRecords: Omit<SiloCapacity, 'id'>[]) => {
      try {
        const currentRecords = await pb.collection('derivative_silo_capacities').getFullList();
        for (const record of currentRecords) {
          await pb.collection('derivative_silo_capacities').delete(record.id);
        }
