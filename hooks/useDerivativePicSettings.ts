...
const CACHE_KEY = CacheKeys.DERIVATIVE_PIC_SETTINGS;
const CACHE_TIME = 10; // Minutes

export const useDerivativePicSettings = () => {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      const result = await safeApiCall(() =>
        pb.collection('derivative_pic_settings').getFullList({
          sort: 'pic',
        })
      );
    ...
    const subscribe = async () => {
      try {
        ...
        unsubPromise = await safeApiCall(() =>
          pb.collection('derivative_pic_settings').subscribe('*', (e) => {
            if (!isSubscribed) return;
  ...
  const addRecord = useCallback(
    async (record: Omit<PicSetting, 'id'>) => {
      try {
        await pb.collection('derivative_pic_settings').create(record);
        cacheManager.delete(CACHE_KEY);
        fetchRecords();
      } catch {
        /* ignore */
  ...
  const updateRecord = useCallback(
    async (updatedRecord: PicSetting) => {
      try {
        ...
        await pb.collection('derivative_pic_settings').update(id, updateData);
        cacheManager.delete(CACHE_KEY);
        fetchRecords();
      } catch {
        /* ignore */
  ...
  const deleteRecord = useCallback(
    async (recordId: string) => {
      try {
        await pb.collection('derivative_pic_settings').delete(recordId);
        cacheManager.delete(CACHE_KEY);
        fetchRecords();
      } catch {
        /* ignore */
