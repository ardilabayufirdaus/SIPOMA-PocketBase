...
const CACHE_KEY = CacheKeys.DERIVATIVE_PLANT_UNITS;
const CACHE_TIME = 10; // Minutes

export const useDerivativePlantUnits = () => {
  ...
  const fetchRecords = useCallback(async () => {
    ...
    try {
      const result = await safeApiCall(() =>
        pb.collection('derivative_plant_units').getFullList({
          sort: 'category,unit',
        })
      );
  ...
  // Enhanced realtime subscription for derivative_plant_units changes
  useEffect(() => {
    ...
    const subscribe = async () => {
      try {
        ...
        unsubPromise = await safeApiCall(() =>
          pb.collection('derivative_plant_units').subscribe('*', (e) => {
            if (!isSubscribed) return;
        ...
        if (!errMsg.includes('autocancelled') && !errMsg.includes('connection')) {
  ...
  const addRecord = useCallback(
    async (record: Omit<PlantUnit, 'id'>) => {
      try {
        await pb.collection('derivative_plant_units').create(record);
  ...
  const updateRecord = useCallback(
    async (updatedRecord: PlantUnit) => {
      try {
        ...
        await pb.collection('derivative_plant_units').update(id, updateData);
        // Clear cache on data change
        cacheManager.delete(CACHE_KEY);
        fetchRecords();
      } catch {
  ...
  const deleteRecord = useCallback(
    async (recordId: string) => {
      try {
        await pb.collection('derivative_plant_units').delete(recordId);
        // Clear cache on data change
        cacheManager.delete(CACHE_KEY);
        fetchRecords();
      } catch {
  ...
  const setAllRecords = useCallback(
    async (newRecords: Omit<PlantUnit, 'id'>[]) => {
      try {
        ...
        const existingRecords = await pb.collection('derivative_plant_units').getFullList();

        // Delete all existing records
        for (const record of existingRecords) {
          await pb.collection('derivative_plant_units').delete(record.id);
        }
