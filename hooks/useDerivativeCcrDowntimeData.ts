...
export const useDerivativeCcrDowntimeData = (date?: string) => {
  ...
  const {
    ...
    queryKey: ['derivative-ccr-downtime-data', date],
    queryFn: async (): Promise<CcrDowntimeData[]> => {
      try {
        ...
        logger.debug('Fetching Derivative downtime data with filter:', filter);
        ...
        try {
          ...
          if (date) {
            ...
            // 1. Exact match
            records = await pb.collection('derivative_ccr_downtime_data').getFullList({
              filter: `date = "${normalizedDate}"`,
              sort: '-created',
              requestKey: `derivative-downtime-exact-${normalizedDate}-${Date.now()}`,
            ...
            if (records.length === 0) {
              records = await pb.collection('derivative_ccr_downtime_data').getFullList({
                filter: `date ~ "${normalizedDate}"`,
                sort: '-created',
                requestKey: `derivative-downtime-flex-${normalizedDate}-${Date.now()}`,
                signal: abortController.signal,
              });
            }
            ...
            if (records.length === 0) {
              const allRecords = await pb.collection('derivative_ccr_downtime_data').getFullList({
                sort: '-created',
                perPage: 100,
                requestKey: `derivative-downtime-all-${Date.now()}`,
                signal: abortController.signal,
              });
        ...
        throw new Error(
          `Failed to fetch Derivative downtime data: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
      }
    },
    staleTime: 20 * 1000,
  ...
  const addDowntimeMutation = useMutation({
    mutationFn: async (record: Omit<CcrDowntimeData, 'id'>) => {
      ...
      await pb.collection('derivative_ccr_downtime_data').create(payload);
      return record;
    },
    onMutate: async (newRecord) => {
      await queryClient.cancelQueries({
        queryKey: ['derivative-ccr-downtime-data', newRecord.date],
      });
      ...
      queryClient.setQueryData(
        ['derivative-ccr-downtime-data', newRecord.date],
    ...
    onError: (err, newRecord, context) => {
      if (context?.previousData) {
        queryClient.setQueryData(
          ['derivative-ccr-downtime-data', context.date],
          context.previousData
        );
      }
    },
    onSuccess: (record) => {
      queryClient.invalidateQueries({ queryKey: ['derivative-ccr-downtime-data', record.date] });
      queryClient.invalidateQueries({ queryKey: ['derivative-ccr-downtime-data'], exact: false });

      setTimeout(() => {
        queryClient.refetchQueries({ queryKey: ['derivative-ccr-downtime-data', record.date] });
        queryClient.refetchQueries({ queryKey: ['derivative-ccr-downtime-data'], exact: false });
      }, 1000);
    },
  });
  ...
  const updateDowntimeMutation = useMutation({
    mutationFn: async (updatedRecord: CcrDowntimeData) => {
      ...
      await pb.collection('derivative_ccr_downtime_data').update(id, payload);
  ...
  const deleteDowntimeMutation = useMutation({
    mutationFn: async (recordId: string) => {
      const record = await pb.collection('derivative_ccr_downtime_data').getOne(recordId);
      if (!record) {
        throw new Error('Record not found for deletion');
      }
      await pb.collection('derivative_ccr_downtime_data').delete(recordId);
      return (record as unknown as { date: string }).date;
    },
    onSuccess: (date) => {
      queryClient.invalidateQueries({ queryKey: ['derivative-ccr-downtime-data', date] });
      queryClient.invalidateQueries({ queryKey: ['derivative-ccr-downtime-data'], exact: false });
    },
  });
  ...
  const debouncedInvalidate = useCallback(
    debounce(() => {
      queryClient.invalidateQueries({ queryKey: ['derivative-ccr-downtime-data'] });
    }, 1000),
    [queryClient]
  );
  ...
  useEffect(() => {
    ...
    pb.collection('derivative_ccr_downtime_data')
      .subscribe('*', () => {
        debouncedInvalidate();
      })
      .then((unsubFunc) => {
  ...
  const getDowntimeForDate = (targetDate: string): CcrDowntimeData[] => {
    ...
    const filtered = downtimeData.filter((d) => {
...
export default useDerivativeCcrDowntimeData;
