...
/**
 * Hook for managing CCR Material Usage data with CRUD operations
 * Following the same pattern as CCR Silo Data Entry
 */
export const useCcrMaterialUsage = () => {
  ...
  const getDataForDatePaginated = useCallback(async (params: MaterialUsageParams) => {
    ...
    try {
      ...
      const result = await pb.collection('ccr_material_usage').getList(page, perPage, {
        filter,
        sort: 'created',
      });
  ...
  const createMaterialUsage = useCallback(async (data: Partial<CcrMaterialUsageSchema>) => {
    try {
      ...
      return await pb.collection('ccr_material_usage').create(data);
    } catch (err) {
      ...
      if (typeof idOrDateOrParams === 'string' && typeof dataOrPlantUnit === 'object') {
        ...
        try {
          ...
          if (
            ...
            const currentRecord = await pb.collection('ccr_material_usage').getOne(id);
            const total =
          ...
          return await pb.collection('ccr_material_usage').update(id, data);
        } catch (err) {
  ...
  const deleteMaterialUsage = useCallback(async (id: string) => {
    try {
      return await pb.collection('ccr_material_usage').delete(id);
    } catch (err) {
  ...
  const saveMaterialUsage = useCallback(async (data: Partial<CcrMaterialUsageSchema>) => {
    ...
    try {
      ...
      if (existingRecords.length > 0) {
        ...
        return await pb.collection('ccr_material_usage').update(record.id, saveData);
      } else {
        // Create new record
        return await pb.collection('ccr_material_usage').create(saveData);
      }
    } catch (err) {
