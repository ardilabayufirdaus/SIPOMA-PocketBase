...
async function createTextOnlyCollection() {
  try {
    ...
    // Create collection with text fields only
    console.log('Creating collection with text fields only...');
    ...
    console.log('✅ COP Footer Parameters collection created with text field (parameter_ids_text)');
    console.log(
      '⚠️  Note: You may need to manually change parameter_ids_text to JSON type in PocketBase admin panel'
    );
  } catch (error) {
