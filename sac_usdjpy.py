...
1. TECHNICAL: Focus on EMA 20/50/200 Trend H4/D1. Tenkan/Kijun (Ichimoku) for entry. SMA 100 as institutional level. RSI (14) Divergence. MACD zero cross. Bollinger Bands (20,2) "walk the band" during strong trend.
2. FUNDAMENTAL: Yield differential US-Japan (BoJ Policy) is the main driver. Yield spread (US10Y-JP10Y). Monitor BoJ policies and Fed stance.
3. SENTIMENT: Safe-haven flows. Interventions risk at Round numbers (150, 155, 160). Carry trade sentiment.
4. RISK CONTROL: ATR H4 50-100 pip. Watch for BoJ intervention spikes (200+ pips). Fibonacci 38.2%/50% pullbacks are common. Weekly Pivot is key.
...
def get_tf_indicators(df, tf_id):
    ...
    ema50 = ta.ema(d['close'], 50).iloc[-1]
    ema20 = ta.ema(d['close'], 20).iloc[-1]
    sma100 = ta.sma(d['close'], 100).iloc[-1]
    ...
    return {
        "tf": tf_id, "price": d['close'].iloc[-1], "ema20": ema20, "ema50": ema50, 
        "ema200": ema200, "sma100": sma100, "rsi": rsi, "macd": macd.to_dict(), 
        "bb": bb.iloc[-1].to_dict() if not bb.empty else {}, "atr": atr, "trend": trend
    }
    ...
    try:
        ...
        if decision.get('action', 'HOLD') != 'HOLD':
            ...
            if alignment and decision['confidence'] >= 83:
                ...
                with open(PATHS['signal'], 'a') as f: f.write(f"{PAIR}|{decision['action']}|{round(decision['entry'], 3)}|{round(t['sl'], 3)}|{round(t['tp'], 3)}|{t['lot']}|{SAC_VERSION}\n")
                ...
                if os.path.exists(PATHS['price_m5']):
                    ...
                    if not d_m5.empty:
                        ...
                        for t in STATS['trades']:
                            ...
                            profit_pts = (curr_p - entry if 'BUY' in side else entry - curr_p) * 1000
                            if profit_pts > (1.1 * atr * 1000):
                                nsl = entry + (0.1 * atr) if 'BUY' in side else entry - (0.1 * atr)
                                if ('BUY' in side and nsl > sl + 0.001) or ('SELL' in side and nsl < sl - 0.001):
                                    with open(PATHS['modify'], 'a') as f: f.write(f"{ticket}|{round(nsl, 3)}|{round(tp, 3)}\n")
