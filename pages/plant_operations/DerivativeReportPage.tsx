...
import { useDerivativeSimpleReportSettings } from '../../hooks/useDerivativeSimpleReportSettings';
import { useDerivativeParameterSettings } from '../../hooks/useDerivativeParameterSettings';
import { useDerivativeCcrParameterDataFlat } from '../../hooks/useDerivativeCcrParameterDataFlat';
import { useDerivativePlantUnits } from '../../hooks/useDerivativePlantUnits';
import { useDerivativeCcrDowntimeData } from '../../hooks/useDerivativeCcrDowntimeData';
import { useDerivativeCcrSiloData } from '../../hooks/useDerivativeCcrSiloData';
import { useDerivativeSiloCapacities } from '../../hooks/useDerivativeSiloCapacities';
import { useDerivativeCcrInformationData } from '../../hooks/useDerivativeCcrInformationData';
import { useDerivativeCcrMaterialUsage } from '../../hooks/useDerivativeCcrMaterialUsage';
...
import { CcrInformationData } from '../../hooks/useDerivativeCcrInformationData';
import {
  formatDateWithDay,
...
const DerivativeReportPage: React.FC<{ t: Record<string, string> }> = ({ t }) => {
  ...
  const { records: reportSettings, loading: reportSettingsLoading } = useDerivativeReportSettings();
  const { records: simpleReportSettings, loading: simpleReportSettingsLoading } =
    useDerivativeSimpleReportSettings();
  const { records: parameterSettings, loading: parameterSettingsLoading } =
    useDerivativeParameterSettings();
  const { getDataForDate } = useDerivativeCcrParameterDataFlat();
  const { records: plantUnits, loading: plantUnitsLoading } = useDerivativePlantUnits();
  const { getDowntimeForDate } = useDerivativeCcrDowntimeData();
  const { getDataForDate: getSiloDataForDate } = useDerivativeCcrSiloData();
  const { records: siloMasterData, loading: siloMasterLoading } = useDerivativeSiloCapacities();
  const { getInformationForDate } = useDerivativeCcrInformationData();
  const { getDataForDate: getMaterialUsageForDate } = useDerivativeCcrMaterialUsage();
  ...
  const simpleReportConfig = useMemo(() => {
    ...
    if (!simpleReportSettingsLoading && simpleReportSettings.length > 0) {
    ...
    // Fallback jika simpleReportSettings belum dikonfigurasi: gunakan semua parameter untuk unit & category tersebut
    if (settingsWithDetails.length === 0) {
      const unitParams = parameterSettings.filter(
        (p) => p.unit === selectedUnit && p.category === selectedCategory
      );
  ...
  const handleGenerateReport = useCallback(async () => {
    ...
    try {
      ...
      const rows = Array.from({ length: 24 }, (_, i) => {
        ...
        allParams.forEach((param) => {
          ...
          if (hourData !== undefined && hourData !== null && hourData !== '') {
            values[param.id] = hourData as string | number;
          } else {
            values[param.id] = '';
          }
        });
      ...
      const dataForReport = {
        ...
        rows,
        footer: footerStats,
        title: `DERIVATIVE PLANT ${t.op_report_title ? t.op_report_title.toUpperCase() : 'DAILY OPERATIONAL REPORT'} - ${selectedUnit.toUpperCase()}`,
  ...
  return (
    <div className="space-y-6 font-sans">
          ...
          <div className="flex flex-wrap items-end gap-4 flex-1">
            ...
            <div className="min-w-[170px] flex-1">
              <label
                ...
                className="flex items-center gap-1.5 text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2"
              >
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-primary-500/40 focus:border-primary-500 text-sm font-medium transition-all duration-200 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer"
                >
            ...
            <div className="min-w-[170px] flex-1">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full appearance-none px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-primary-500/40 focus:border-primary-500 disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed text-sm font-medium transition-all duration-200 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer"
                >
            ...
            <div className="min-w-[150px] flex-1">
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent text-sm font-medium transition-all duration-200 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer"
              />
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {isLoading
                  ? (t.processing || 'PROCESSING...').toUpperCase()
              ...
              >
                {(t.simple_report || 'SIMPLE').toUpperCase()}
              </EnhancedButton>
            </div>
          </div>
          ...
          {reportData && (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                ...
                {isCopying
                  ? (t.copying || 'COPYING...').toUpperCase()
                  : copySuccess
                    ? (t.copied || 'COPIED!').toUpperCase()
                    : (t.copy_image || 'COPY IMAGE').toUpperCase()}
              </EnhancedButton>
              ...
              >
                ...
                {isExportingPDF
                  ? (t.exporting_pdf || 'EXPORTING...').toUpperCase()
                  : (t.export_pdf || 'EXPORT PDF').toUpperCase()}
              </EnhancedButton>
            </div>
          )}
        </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-900 p-8 rounded-xl min-h-[60vh] flex items-center justify-center border border-slate-200 dark:border-slate-800">
        ...
        {isLoading && (
          <div className="text-center">
            ...
            <p className="text-xl font-bold text-primary-600 dark:text-primary-400 animate-pulse uppercase tracking-widest">
              {t.generating_report_message || 'GENERATING REPORT...'}
            </p>
          </div>
        )}
        {reportData && !isLoading && (
          <div ref={reportRef} className="w-full max-w-full bg-white dark:bg-slate-900 shadow-xl">
            <InteractiveReport
        ...
        {!isLoading && !reportData && reportConfig.length > 0 && (
          <div className="text-center max-w-lg">
            <div className="w-24 h-24 bg-white dark:bg-slate-800 rounded-full flex items-center justify-center mb-6 mx-auto shadow-sm border border-slate-200 dark:border-slate-700">
              <svg
            ...
            <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2">
              {t.no_report_generated || 'No Report Generated'}
            </h3>
            <p className="text-slate-500 dark:text-slate-400">
              {t.click_generate_report || 'Click GENERATE LOG SHEET to view report.'}
            </p>
          </div>
        )}
      </div>
      ...
      {isCopying && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl p-8 flex flex-col items-center gap-4 min-w-[300px] border-t-4 border-primary-600">
            <LoadingSpinner size="lg" className="border-primary-600" />
            <div className="text-center">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">
                {t.copying || 'Copying Report Image'}
              </h3>
...
export default DerivativeReportPage;
