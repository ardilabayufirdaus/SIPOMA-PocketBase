    ...
    >
      ...
      <div className="overflow-x-auto max-w-full">
        <table className="w-full text-[6px] min-w-max">
          <thead>
            <tr className="bg-gradient-to-r from-blue-100 to-cyan-100 dark:from-blue-900/30 dark:to-cyan-900/30">
              ...
              {materialColumns.map((col) => (
                <th
                  key={col.key}
                  className="px-1 py-1 text-left font-bold text-slate-800 dark:text-slate-200 border-r border-blue-200 dark:border-blue-700 align-middle text-[6px]"
                >
                  {col.label}
                </th>
              ))}
          ...
          <tbody className="bg-white dark:bg-slate-800 divide-y divide-blue-200 dark:divide-blue-700">
              ...
              sortedMaterialUsageData.map((row, index) => (
                ...
                >
                  ...
                  {materialColumns.map((col) => (
                    <td
                      key={col.key}
                      className="px-1 py-1 text-slate-800 dark:text-slate-200 border-r border-blue-200 dark:border-blue-700 align-middle text-[6px]"
                    >
          ...
          {sortedMaterialUsageData.length > 0 && (
            <tfoot className="bg-blue-100 dark:bg-blue-900/30">
              <tr>
                <td className="px-1 py-1 text-[6px] font-semibold text-slate-900 dark:text-slate-100 border-r border-blue-200 dark:border-blue-700">
                  Total
                </td>
                {materialColumns.map((col) => (
                  <td
                    key={col.key}
                    className="px-1 py-1 text-[6px] font-semibold text-slate-900 dark:text-slate-100 border-r border-blue-200 dark:border-blue-700"
                  >
