...
const RkcCcrDataEntryPage: React.FC<{ t: Record<string, string> }> = ({ t }) => {
  ...
  const fetchProfiles = useCallback(async () => {
    try {
      ...
      // Fetch RKC profiles and standard profiles
      let records: any[] = [];
      try {
        records = await pb.collection('rkc_parameter_order_profiles').getFullList({
      ...
      // If no RKC profiles, check standard parameter_order_profiles as fallback
      if (records.length === 0) {
        try {
  ...
  const deleteProfile = useCallback(
    async (profile: ParameterProfile) => {
      ...
      try {
        try {
          await pb.collection('rkc_parameter_order_profiles').delete(profile.id);
        } catch {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (allFooterDataToSave.length > 0) {
          ...
          // Clear indexedDB cache for this unit/date so RKC COP Analysis fetches fresh data
          try {
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <h1 className="text-xl sm:text-2xl font-black tracking-tight text-white font-display">
                RKC {t.op_ccr_data_entry || 'CCR Data Entry'}
              </h1>
              <p className="text-xs text-slate-300 font-medium mt-0.5">
                {t.ccr_page_description ||
                  'Real-time CCR log sheet input, hourly parameter monitoring & silo tracking (RKC)'}
              </p>
            </div>
          </div>
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="flex-1 min-w-[150px]">
            ...
            <div className="relative group/rkc-date">
              <div className="w-full px-3 py-1.5 h-[36px] min-h-[36px] bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-800 dark:text-slate-100 text-xs font-medium flex items-center justify-between pointer-events-none group-hover/rkc-date:border-slate-300 dark:group-hover/rkc-date:border-slate-600">
                <span>
            ...
            <div className="flex items-center gap-2 flex-wrap justify-end">
              ...
              {hasPermission('rkc_plant_operations', 'WRITE') && selectedUnit && (
                <div className="flex items-center gap-1.5">
                  ...
                  {/* AI Shift Report */}
                  {selectedDate && <ShiftHandoverButton date={selectedDate} unit={selectedUnit} />}
                </div>
              )}
            </div>
                  ...
                  {canAccessMonthlyExportImport(loggedInUser?.role || pb.authStore.model?.role) && (
                    <>
                      ...
                      <button
                        ...
                        className="min-h-[34px] h-[34px] px-2.5 inline-flex items-center gap-1.5 text-xs font-semibold rounded-lg bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white shadow-xs focus:outline-none focus:ring-2 focus:ring-emerald-500/40 transition-all"
                    ...
                    {filteredParameterSettings.length > 0 ? (
                      Array.from({ length: 24 }, (_, i) => i + 1).map((hour) => (
                        ...
                        >
                          ...
                          <td
                            className={`px-2 py-1.5 whitespace-nowrap text-xs text-slate-600 dark:text-slate-300 border-r border-slate-200/60 dark:border-slate-800 sticky left-[60px] z-20 ${
                              hour % 2 === 0
                                ? 'bg-white dark:bg-slate-900'
                                : 'bg-slate-50 dark:bg-slate-850'
                            } group-hover:bg-slate-100 dark:group-hover:bg-slate-800`}
                              ...
                              <React.Fragment key={shift}>
                                ...
                                >
                                  <input
                                    ...
                                    aria-label={`${t.empty_space} ${masterSilo.silo_name} ${shift}`}
                                    disabled={!canWrite}
                                    title={`${t.fill_empty_space} ${masterSilo.silo_name} ${t.shift} ${i + 1}`}
                                    placeholder="0,0"
                                  />
                                </td>
                                <td
                                ...
                                >
                                  <input
                                    ...
                                    className="w-full text-center px-1.5 py-1 text-xs font-mono bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-slate-700 rounded focus:outline-none focus:ring-1 focus:ring-primary-500 disabled:opacity-40"
                                    aria-label={`${t.content} ${masterSilo.silo_name} ${shift}`}
                                    disabled={!canWrite}
                                    title={`${t.fill_content} ${masterSilo.silo_name} ${t.shift} ${i + 1} (Max: ${masterSilo.capacity})`}
                                    placeholder="0,0"
                                  />
                                </td>
                                <td
            ...
            <div className="flex justify-between items-center flex-wrap gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <span className="text-xs font-semibold text-white">{t.add_downtime_button}</span>
                </EnhancedButton>
              </div>
            </div>
            <div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-800 shadow-xs">
