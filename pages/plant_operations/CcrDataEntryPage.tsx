        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <h1 className="text-xl sm:text-2xl font-black tracking-tight text-white font-display">
                {t.op_ccr_data_entry || 'CCR Data Entry'}
              </h1>
              <p className="text-xs text-slate-300 font-medium mt-0.5">
                {t.ccr_page_description ||
                  'Real-time CCR log sheet input, hourly parameter monitoring & silo tracking'}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2.5 self-start md:self-auto bg-slate-800/80 backdrop-blur-md px-3 py-1.5 rounded-lg border border-slate-700/80 shadow-xs">
            <RealtimeIndicator
              ...
              lastUpdate={
                offlineSync.lastSyncTime ? new Date(offlineSync.lastSyncTime) : new Date()
              }
              ...
              >
                <div className="ccr-table-wrapper">
                  ...
                  >
                    ...
                    <thead
                      className="bg-slate-700 dark:bg-slate-800 text-white text-center sticky top-0 z-20 shadow-xs border-b border-slate-600 dark:border-slate-700"
                      role="rowgroup"
                    >
                      <tr className="border-b border-slate-600/60" role="row">
                        <th
                          rowSpan={2}
                          className="px-2 py-2 text-center text-[11px] font-bold text-white uppercase tracking-wider border-r border-slate-600 sticky left-0 top-0 bg-slate-800 dark:bg-slate-900 z-30 align-middle"
                        ...
                        <th
                          rowSpan={2}
                          className="px-2 py-2 text-center text-[11px] font-bold text-white uppercase tracking-wider border-r border-slate-600 sticky left-[60px] top-0 bg-slate-800 dark:bg-slate-900 z-30 align-middle"
                        ...
                        <th
                          rowSpan={2}
                          className="px-3 py-2 text-center text-[11px] font-bold text-white uppercase tracking-wider border-r border-slate-600 sticky left-[140px] top-0 bg-slate-800 dark:bg-slate-900 z-30 shadow-[2px_0_4px_-1px_rgba(0,0,0,0.2)] align-middle"
                        ...
                        {filteredParameterSettings.map((param) => (
                          <th
                            key={param.id}
                            className={`px-2 py-2 text-[11px] font-bold border-r border-slate-600 text-center bg-slate-700 dark:bg-slate-800 text-white ${
                          ...
                          >
                            <div className="text-center">
                              <div className="font-bold text-[11px] leading-tight uppercase tracking-wider text-white">
                                {param.parameter}
                              </div>
                            </div>
                          </th>
                      ...
                      <tr
                        className="border-b border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-850"
                        role="row"
                      >
                        {filteredParameterSettings.map((param) => (
                          <th
                            key={`minmax-${param.id}`}
                            className={`px-1.5 py-1 text-[10px] font-mono border-r border-slate-200 dark:border-slate-700 text-center bg-slate-100 dark:bg-slate-850 text-slate-600 dark:text-slate-400 ${
                              shouldHighlightColumn(param) ? 'filtered-column' : ''
                            }`}
                            style={{ width: '80px', minWidth: '80px' }}
                            role="columnheader"
                    ...
                    <tbody className="bg-white dark:bg-slate-900" role="rowgroup">
                      {filteredParameterSettings.length > 0 ? (
                        Array.from({ length: 24 }, (_, i) => i + 1).map((hour) => (
                          <tr
                            key={hour}
                            className={`border-b border-slate-200/60 dark:border-slate-800 group ${
                              hour % 2 === 0
                                  ...
                                  if (isSuperAdmin(loggedInUser?.role)) {
                                    return (
                                      <input
                                        ...
                                        title={`Edit nama operator jam ${hour}`}
                                        disabled={!canWrite}
                                      />
                                    );
                                  } else {
        ...
        <div className="space-y-4 sm:space-y-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3.5 sm:gap-4">
            ...
            <div className="relative group">
              <EnhancedCard className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xs p-3.5 sm:p-4 space-y-3.5">
                <div className="flex items-center gap-3">
                    ...
                    <tbody className="bg-white dark:bg-slate-900 divide-y divide-slate-200 dark:divide-slate-800">
                      {dailySiloData.length > 0 &&
                        dailySiloData.map((siloData, siloIndex) => {
                          ...
                          return (
                            <tr
                              key={siloData.id || `silo-${siloIndex}`}
                              className="hover:bg-slate-50 dark:hover:bg-slate-850 transition-colors"
                            >
                              <td className="px-3 py-1.5 whitespace-nowrap text-xs font-semibold text-slate-800 dark:text-slate-200 border-r border-slate-200 dark:border-slate-800">
                                {masterSilo.silo_name}
                              </td>
                              {[1, 2, 3].map((shiftNum) => {
                                const shift = `shift${shiftNum}` as 'shift1' | 'shift2' | 'shift3';
                                const content = siloData[shift]?.content;
                                const percentage =
                                  content !== undefined && masterSilo.capacity > 0
                                    ? (content / masterSilo.capacity) * 100
                                    : 0;

                                const i = shiftNum - 1;

                                return (
                                  <React.Fragment key={`shift-${shiftNum}`}>
