...
import { RkcReportSetting, ParameterSetting, ParameterDataType } from '../../types';
...
interface FormProps {
  recordToEdit: RkcReportSetting | null;
  onSave: (record: RkcReportSetting | Omit<RkcReportSetting, 'id'>) => void;
  onCancel: () => void;
...
const RkcReportSettingForm: React.FC<FormProps> = ({
  recordToEdit,
  onSave,
  onCancel,
  t,
  ...
  const availableParameters = useMemo(() => {
    return (
      allParameters
        ...
        // Note: Check if data_type matches ParameterDataType enum or string 'Number'
        .filter(
          (p) =>
            (p.data_type as string) === ParameterDataType.NUMBER ||
            (p.data_type as string) === 'Number'
        )
  ...
  const validateField = useCallback(
    (name: keyof FormData, value: string | number): string => {
      switch (name) {
        case 'parameter_id':
          if (!value) return t.required_field || 'Parameter is required';
          if (!availableParameters.find((p) => p.id === value))
            return t.invalid_parameter || 'Selected parameter is not available';
          return '';
        case 'category': {
          if (!value) return t.required_field || 'Category is required';
          const strValue = String(value);
          if (strValue.trim().length < 2)
            return t.min_length_error || 'Category must be at least 2 characters';
          if (strValue.trim().length > 50)
            return t.max_length_error || 'Category must be less than 50 characters';
          return '';
        }
        case 'order':
          return '';
        ...
        >
          ...
          >
            <label htmlFor="parameter_id" className="block text-sm font-medium text-[#333333] mb-2">
              {t.parameter_select_label || 'Select Parameter'}
            ...
            >
              <option value="">
                {availableParameters.length === 0
                  ...
                  : t.parameter_select_placeholder || 'Select a parameter...'}
              </option>
          ...
          >
            <label htmlFor="category" className="block text-sm font-medium text-[#333333] mb-2">
              {t.report_category_label || 'Report Category'}
              <span className="text-[#C7162B] ml-1">*</span>
            </label>
            <motion.input
              whileFocus={{ scale: 1.02 }}
...
export default RkcReportSettingForm;
