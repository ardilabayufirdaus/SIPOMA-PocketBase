...
const DerivativeCopAnalysisPage: React.FC<{ t: Record<string, string> }> = ({ t }) => {
  ...
  const { copFooterConfigs, copFooterParameterIds } = useDerivativeCopFooterParameters(
    selectedCategory,
    selectedUnit
  );
    ...
    const fetchComparisonData = async () => {
      ...
      try {
        ...
        const comparisonAnalysisData = filteredCopParameters
          .map((parameter) => {
            const dailyValues: ChartDataItem[] = comparisonDates.map((dateString, dayIndex) => {
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300 font-medium">
                Monitoring kepatuhan parameter operasional, indeks QAF, dan performa unit derivative
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 self-stretch sm:self-auto">
            <button
              ...
              className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-2 px-4 py-2.5 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 rounded-xl shadow-sm hover:shadow transition-all min-h-[38px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
              title="Export Data Analisis ke Excel (.xlsx)"
            >
